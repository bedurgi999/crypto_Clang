    ...
    while (1)
    {
        ...
        if (pid == 0) // child process
        {
            ...
            while(msg_type != LOGIN_SUCCESS)
            {
                ...
                pt_id_len = decrypt(id.payload, id.msg_len, key, iv, (unsigned char *)recv_id);
                pt_pw_len = decrypt(pw.payload, pw.msg_len, key, iv, (unsigned char *)recv_pw);
            ...
            while (msg_type != QUIT)
            {
                ...
                case UP: //실행파일, C파일이 존재하는 폴더에 있는 파일만 업로드 가능
                    ...
                    msg_in.msg_len = ntohl(msg_in.msg_len);
                    ciphertext_len = decrypt(msg_in.payload, msg_in.msg_len, key, iv, (unsigned char *)file_name2);
                ...
                case DOWN: // 서버의 파일을 클라이언트가 다운로드, 실행파일, C파일이 존재하는 폴더에 있는 파일만 다운로드 가능
                    ...
                    readn(clnt_sock, hash1, sizeof(hash1));
                    msg_in.msg_len = ntohl(msg_in.msg_len);
                    ciphertext_len = decrypt(msg_in.payload, msg_in.msg_len, key, iv, (unsigned char *)file_name1);
                ...
                case LIST:
                    ...
                    if (dir != NULL)
                    {
                        ...
                        while ((ent = readdir(dir)) != NULL)
                        {
                            ...
                            len = encrypt((unsigned char*)file_name, len, key, iv, msg_out.payload);
