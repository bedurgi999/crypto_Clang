...
int main(int argc, char* argv[])
{
    ...
    {  
        ...
        case INPUT_IDPW: // ID/PW 입력 부분
            ...
            ct_id_len = encrypt((unsigned char *)id_array, ct_id_len, key, iv, id.payload);
            id.type = msg_type;
            id.type = htonl(id.type);
            id.msg_len = htonl(ct_id_len);
            
            M_SHA256(pw_array, strlen(pw_array), hash2);
            ct_pw_len = encrypt((unsigned char *)pw_array, ct_pw_len, key, iv, pw.payload);
            pw.type = msg_type;
        ...
        case LOGIN_FAIL:
            ...
            if (select == 1)
            {
                ...
                ct_id_len = encrypt((unsigned char *)id_array, id.msg_len, key, iv, id.payload);
                ct_pw_len = encrypt((unsigned char *)pw_array, pw.msg_len, key, iv, pw.payload);
    ...
    while(msg_type != QUIT)
    {
        ...
        case UP: // 파일 업로드 실행파일, C파일이 존재하는 폴더에 있는 파일만 업로드 가능
            ...
            while (1)
            {
                ...
                file_len = encrypt((unsigned char*)buf, file_len, key, iv, msg_out.payload);
        ...
        case DOWN: // 파일 다운로드 실행파일, C파일이 존재하는 폴더에있는 파일만 다운로드 가능
            ...
            fd = open(file_name2, O_CREAT | O_WRONLY | O_TRUNC, S_IRWXU);
            ...
            while (1)
            {
                ...
                if (msg_in.type == FILE_DATA)
                {
                    ...
                    file_len = decrypt(msg_in.payload, msg_in.msg_len, key, iv, (unsigned char *)buf);
        ...
        case LIST: // savedata 폴더의 파일 리스트 전송
            ...
            while(1)
            {
                ...
                if (msg_in.type != SEND_FINISH)
                {
                    ...
                    readn(sock, hash1, sizeof(hash1));
                    ciphertext_len = decrypt(msg_in.payload, msg_in.msg_len, key, iv, (unsigned char*)dec_filename);
                ...
                else if (msg_in.type == SEND_FINISH) // 리스트 전송 끝
                {
                    msg_type = WAIT;
                    break;
                }
